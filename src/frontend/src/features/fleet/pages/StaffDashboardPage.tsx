...
export function StaffDashboardPage() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="font-display text-3xl font-bold text-[#131b2e]">Staff Dispatch Dashboard</h2>
        <p className="text-sm text-[#505f76]">Assign paid orders to available fleet capacity and monitor shipment records.</p>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <Card padding={false}>
          <CardHeader
            ...
            action={<span className="font-mono text-xs text-[#505f76]">{processingOrders.length} ORDERS</span>}
          />
          <div className="p-4">
            {loadingOrders ? (
              ...
              <ul className="space-y-3">
                {processingOrders.map(order => (
                  <li key={order.id} className="p-4 rounded-md border border-[#c3c6d7] bg-[#faf8ff]">
                    <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                      <div className="space-y-2">
                        <div className="flex flex-wrap items-center gap-3">
                          <span className="font-mono text-xs font-semibold text-[#131b2e]">{order.id.toUpperCase()}</span>
                          <StatusBadge type="order" status={order.status} />
                          <span className="text-xs text-[#505f76]">{order.isPaid ? 'Paid' : 'Unpaid'}</span>
                        </div>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1 text-xs text-[#505f76]">
                          <p>Customer: <span className="font-medium text-[#131b2e]">{getCustomerName(order.customerId)}</span></p>
                          <p>Weight: <span className="font-medium text-[#131b2e]">{order.totalWeightKg}kg</span></p>
                          <p>Cargo: <span className="font-medium text-[#131b2e]">{order.cargoItems.map(item => CARGO_TYPE_LABELS[item.cargoType]).join(', ')}</span></p>
                          <p>Dimensions: <span className="font-medium text-[#131b2e]">{dimensions(order)}</span></p>
                          <p>Payment: <span className="font-medium text-[#131b2e]">{PAYMENT_METHOD_LABELS[order.paymentMethod]}</span></p>
                          <p>Total: <span className="font-medium text-[#131b2e]">{formatCurrency(order.totalAmount)}</span></p>
                        </div>
                      </div>
        ...
        <Card padding={false}>
          <CardHeader
            ...
            action={<span className="font-mono text-xs text-[#505f76]">{activeShipments.length} SHIPMENTS</span>}
          />
          <div className="p-4">
            {loadingShipments ? (
              <div className="py-8 flex justify-center"><Spinner /></div>
                    ...
                    <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                      <div className="space-y-2">
                        <div className="flex flex-wrap items-center gap-3">
                          ...
                          <span className="font-mono text-[11px] text-[#737686]">{shipment.orderId.toUpperCase()}</span>
                        </div>
                        <p className="text-sm font-semibold text-[#131b2e]">{shipment.order?.origin} to {shipment.order?.destination}</p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1 text-xs text-[#505f76]">
                          <p>Customer: <span className="font-medium text-[#131b2e]">{shipment.order ? getCustomerName(shipment.order.customerId) : '-'}</span></p>
                          <p>Weight: <span className="font-medium text-[#131b2e]">{shipment.order?.totalWeightKg}kg</span></p>
                          <p>Dimensions: <span className="font-medium text-[#131b2e]">{shipment.order ? dimensions(shipment.order) : '-'}</span></p>
                          <p>Total: <span className="font-medium text-[#131b2e]">{formatCurrency(shipment.order?.totalAmount ?? 0)}</span></p>
                          <p>Vehicle: <span className="font-medium text-[#131b2e]">{shipment.vehicle?.plateNumber}</span></p>
                          <p>Driver: <span className="font-medium text-[#131b2e]">{shipment.driver?.name}</span></p>
                        </div>
                      </div>
                    </div>
                  </li>
  ...
  return (
    ...
    >
      <div className="space-y-4">
        <div className="rounded-md border border-[#c3c6d7] bg-[#faf8ff] p-3 text-sm">
          <p className="font-semibold text-[#131b2e]">{order.origin} to {order.destination}</p>
          <p className="text-xs text-[#505f76]">{order.totalWeightKg}kg required capacity · {formatCurrency(order.totalAmount)} · {dimensions(order)}</p>
        </div>
